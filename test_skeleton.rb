...
class TestSkeleton
  ...
  def high_and_low(test_string)
    test_string = test_string.to_s.split(' ').map(&:to_i).sort.reverse
    test_string[0].to_s+" "+test_string[-1].to_s
  end
  ...
  def remove_array_elements(source_array, values_array)
    values_array.each do |i|
      source_array.each do |j|
        if i == j
          source_array.delete(j)
        end
      end    
    end
    return source_array
  ...
  def consecutive_duplicates(string)
    ...
    while i < new_arr.length
      if  new_arr[i] != new_arr[i+1]
        result_str= result_str.push(new_arr[i])
      end
      i+=1
    end
    result_str =result_str.map { |i| i.to_s }.join(" ")
    return result_str
  end
  ...
  def middle_chars(test_string)
    ...
    str_len % 2 != 0 ? test_string[(str_len /2).round(half: :up)] : test_string[str_len/2 - 1] + test_string[str_len/2]
  end
end
